...
export class NotificationsService {
  ...
  async sendBulk(
    ...
    if (uniqueRecipients.length === 0) {
      this.logger.warn('Envío masivo sin destinatarios válidos');
      return {
        total: 0,
        sent: 0,
        failed: 0,
  ...
  async notifyIncidentCreated(
    ...
    const messages = recipients.map((to) => ({ to, subject, html, attachments }));
  ...
  async verifyMailProvider(): Promise<{ ok: boolean }> {
    return { ok: await this.mailerService.verifyProvider() };
  }
  ...
  async diagnoseSMTP(): Promise<Record<string, unknown>> {
    ...
    try {
      connection = (await this.mailerService.verifyProvider()) ? 'ok' : 'failed';
    } catch (err) {
